...
public class EventosLogIn extends Control implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try{
			...
			estadoServidor.setImage(new Image(getClass().getResourceAsStream("/com/pasapalabra/game/images/desconectado.png")));
			a.printStackTrace();
	...
	public void loginSession(MouseEvent event){
		...
		if(txtUsuario.getText().length()>0&&txtContra.getText().length()>0){
			...
			try {
				com.pasapalabra.game.utilidades.Conexion_cliente.lanzaConexion(com.pasapalabra.game.utilidades.Conexion_cliente.Ip_Local, com.pasapalabra.game.utilidades.Acciones_servidor.Log.toString(), Datos_cliente);
				if(com.pasapalabra.game.utilidades.Conexion_cliente.Datos_Usuario.get(5)!=null){
					java.io.File file = new java.io.File(com.pasapalabra.game.utilidades.Conexion_cliente.Datos_Usuario.get(5));
					if(file.exists()){
						iAvatar=new Image("file:///"+com.pasapalabra.game.utilidades.Conexion_cliente.Datos_Usuario.get(5));
					}
					else{
	...
	public void registro(MouseEvent event){
		...
		if( servidorOperativo == false ){
			...
			com.pasapalabra.game.utilidades.deVentana.transicionVentana("Registro", event);
		}
	}
